...
class CodacyAPIClient:
    ...
    def list_repositories(self, provider: str, organization: str) -> List[Dict[str, Any]]:
    ...
    def get_organization_integration_settings(self, provider: str, organization: str) -> Dict[str, Any]:
    ...
    def get_repository_integration_settings(self, provider: str, organization: str, repository: str) -> Dict[str, Any]:
    ...
    def update_organization_integration_settings(self, provider: str, organization: str, settings: Dict[str, Any]) -> Dict[str, Any]:
        """Update organization-level integration settings"""
        endpoint = f"/organizations/{provider}/{organization}/integrations/providerSettings"
        response = self._make_request('PATCH', endpoint, json=settings)
        return response.json()
    
    def update_repository_integration_settings(self, provider: str, organization: str, repository: str, settings: Dict[str, Any]) -> Dict[str, Any]:
        """Update repository-level integration settings"""
        endpoint = f"/organizations/{provider}/{organization}/repositories/{repository}/integrations/providerSettings"
        response = self._make_request('PATCH', endpoint, json=settings)
        return response.json()
    
    def refresh_provider_integration(self, provider: str, organization: str, repository: str) -> Dict[str, Any]:
...
class IntegrationManager:
    ...
    def get_current_settings(self, provider: str, organization: str, repository: str) -> Optional[Dict[str, Any]]:
        """Get current integration settings for a repository"""
        try:
            return self.client.get_repository_integration_settings(provider, organization, repository)
        except requests.exceptions.RequestException as e:
            print(f"Failed to get settings for {repository}: {e}")
            return None
    
    def update_integration_settings(self, provider: str, organization: str, repository: str, custom_settings: Optional[Dict[str, Any]] = None) -> bool:
        """Update integration settings for a repository"""
        try:
            # Get default settings for the provider
            default_settings = self.PROVIDER_SETTINGS.get(provider, self.PROVIDER_SETTINGS['gh'])
            ...
            # Update the settings
            result = self.client.update_repository_integration_settings(
    ...
    def refresh_integration(self, provider: str, organization: str, repository: str) -> bool:
        """Refresh provider integration for a repository"""
        try:
            print(f"Refreshing integration for {repository}...")
            self.client.refresh_provider_integration(provider, organization, repository)
    ...
    def reintegrate_repository(self, provider: str, organization: str, repository: str, custom_settings: Optional[Dict[str, Any]] = None) -> bool:
        ...
        current_settings = self.get_current_settings(provider, organization, repository)
        if current_settings:
            print(f"Current settings: {current_settings.get('settings', {})}")
        
        # Update integration settings
        settings_updated = self.update_integration_settings(provider, organization, repository, custom_settings)
        
        # Refresh the integration
        integration_refreshed = self.refresh_integration(provider, organization, repository)
    ...
    def reintegrate_all(self, provider: str, organization: str, target_repositories: Optional[List[str]] = None, custom_settings: Optional[Dict[str, Any]] = None) -> Dict[str, bool]:
        ...
        try:
            repositories = self.client.list_repositories(provider, organization)
        ...
        if target_repositories:
            repositories = [repo for repo in repositories if repo['name'] in target_repositories]
        ...
        for repo in repositories:
            repo_name = repo['name']
            success = self.reintegrate_repository(provider, organization, repo_name, custom_settings)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Codacy Integration Helper - API Version')
    ...
    parser.add_argument('--commit-status', dest='commit_status', action='store_true',
                        help='Enable commit status checks')
    parser.add_argument('--no-commit-status', dest='commit_status', action='store_false',
                        help='Disable commit status checks')
    parser.add_argument('--pr-comments', dest='pr_comments', action='store_true',
                        help='Enable pull request comments')
    parser.add_argument('--no-pr-comments', dest='pr_comments', action='store_false',
    ...
    parser.add_argument('--no-pr-summary', dest='pr_summary', action='store_false',
                        help='Disable pull request summary')
    parser.add_argument('--suggestions', dest='suggestions', action='store_true',
                        help='Enable suggestions (GitHub only)')
    parser.add_argument('--no-suggestions', dest='suggestions', action='store_false',
                        help='Disable suggestions')
    
    parser.set_defaults(commit_status=None, pr_comments=None, pr_summary=None, suggestions=None)
    ...
    if args.provider not in valid_providers:
        print(f"Error: Invalid provider '{args.provider}'. Must be one of: {', '.join(valid_providers)}")
        sys.exit(1)
